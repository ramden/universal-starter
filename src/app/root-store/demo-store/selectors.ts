...
export const selectJokeState: MemoizedSelector<object, State> = createFeatureSelector<State>(
    'joke'
);

export const selectAllJokeItems: (state: object) => Demo[] = featureAdapter.getSelectors(
    selectJokeState
).selectAll;

export const selectJokeById = (id: string) =>
    createSelector(this.selectAllJokeItems, (allJokes: Demo[]) => {
        if (allJokes) {
